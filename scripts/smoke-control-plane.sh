...
project_json="$(curl --fail --silent -X POST "${BASE_URL}/api/v1/projects" \
  -H 'Content-Type: application/json' \
  -d '{"name":"smoke-platform","description":"compose smoke"}')"
project_id="$(json_field "${project_json}" id)"

api_json="$(curl --fail --silent -X POST "${BASE_URL}/api/v1/projects/${project_id}/apis" \
  -H 'Content-Type: application/json' \
  -d '{"name":"orders-api","host":"api.autoapi.local","basePath":"/"}')"
api_id="$(json_field "${api_json}" id)"

pool_json="$(curl --fail --silent -X POST "${BASE_URL}/api/v1/apis/${api_id}/upstream-pools" \
  -H 'Content-Type: application/json' \
  -d '{"name":"orders-v1","loadBalancing":"ROUND_ROBIN"}')"
pool_id="$(json_field "${pool_json}" id)"

curl --fail --silent -X POST "${BASE_URL}/api/v1/upstream-pools/${pool_id}/targets" \
  -H 'Content-Type: application/json' \
  -d '{"url":"http://upstream-v1:8080","enabled":true,"weight":1}' >/dev/null

curl --fail --silent -X POST "${BASE_URL}/api/v1/apis/${api_id}/routes" \
  -H 'Content-Type: application/json' \
...
validate_json="$(curl --fail --silent -X POST "${BASE_URL}/api/v1/apis/${api_id}/config/validate")"
...
version_json="$(curl --fail --silent -X POST "${BASE_URL}/api/v1/apis/${api_id}/config/versions" \
  -H 'Content-Type: application/json' \
...
curl --fail --silent "${BASE_URL}/api/v1/apis/${api_id}/config/versions/1" | grep -q pathPrefix
